    ...
    checkIE: function() {
        ...
        if (version && version[1]) {
            ...
            if ((browser == "Microsoft Internet Explorer" && trim_Version == "MSIE9.0") || (browser == "Microsoft Internet Explorer" && trim_Version == "MSIE8.0")) {
                $('head').append('<link rel="stylesheet" href="/static/css/hack.css">');
            }
        }
    },
...
var APP = {
    ...
    toggleBottom: function(control) {
        if (control) { //开
            $('.main_page').removeClass('hide_main_bottom');
        } else { //关
            $('.main_page').addClass('hide_main_bottom');
        }
    },
    main: function(initTabArr) {
        ...
        // 顶部导行
        $('.main_page .main_top .main_menu li').click(function() {
            var name = $(this).data('name');
            var type = $(this).data('type');

            // 显示左侧
            $('.main_left .toShow').trigger('click');
            if (type == 'hideLeft') { //隐藏左侧
