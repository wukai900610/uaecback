...
/*!
 * _path 获取组件核心文件Box.js所在的绝对路径
 * _args 获取Box.js文件后的url参数组，如：Box.js?self=true&skin=aero中的?后面的内容
 */
_args, _path = (function( script, i, me )
{
	...
	for( ; i < l; i++ )
	{
		...
		if( me.substr(me.lastIndexOf('/')).indexOf('Box') !== -1 )
		    break;
	}
 ...
 * @param  {String}
 * @return {String||null}
 * @demo Box.js?skin=aero | _getArgs('skin') => 'aero'
 */
...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	_init: function( config )
	{
		...
		if( config.icon && !isIfr )
		{
			...
			DOM.icon[0].innerHTML = '<img src="' + config.path + 'Box/icons/' + config.icon + '" class="ui_icon_bg"/>';
		}
		else
	...
	_ie6PngFix: function()
	{
	    if( _ie6 )
		{
			var i = 0, elem, png, pngPath, runtimeStyle,
				path = lhgdialog.setting.path + '/Box/',
...
lhgdialog.tips = function( content, time, icon, callback )
{
	var reIcon = icon ? function(){
		this.DOM.icon[0].innerHTML = '<img src="' + this.config.path + 'Box/icons/' + icon + '" class="ui_icon_bg"/>';
...
function _Box(title, iframe, tIndex, width, height) {
    $.dialog({
        title:title,
        //title: false,
        content: 'url:' + iframe + '',
...
function Box(title, iframe, tIndex, width, height) {
    if (arguments.length == 2) {
        _Box(title, iframe, 1000, 900, 600);
    }
    else if (arguments.length == 3) {
        _Box(title, iframe, tIndex, 900, 600);
    }
    else if (arguments.length == 4) {
        _Box(title, iframe, 1000, tIndex, width);
    }
    else if (arguments.length == 5) {
        _Box(title, iframe, tIndex, width, height);
    }
}

function ShowBox(title, iframe, width, height) {
    if (arguments.length == 2) {
        _Box(title, iframe, 1000, 900, 600);
    }
    else if (arguments.length == 4) {
        _Box(title, iframe, 1000, width, height);
    }
}

function SmallBox(title, iframe) {
    _Box(title, iframe, 1000, 600, 400);
}

function MiniBox(title, iframe) {
    _Box(title, iframe, 1000, 500, 300);
}

function InnerCloseBox() {
...
function ShowTips(strIcon,strContent) {
...
function ShowTipsHref(strIcon,strContent,strURL) {
    $.dialog({
		title: false,
		zIndex:2000,
        icon: strIcon,
...
function PopUpTipsHref(strIcon,strContent,strURL) {
    $.dialog({
		title: false,
		zIndex:2000,
        icon: strIcon,
